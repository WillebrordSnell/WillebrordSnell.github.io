...
**MOTIVATION：** 作者认为现有很多非受限攻击的共同痛点是：容易引入不自然的颜色、污渍、色彩方案冲突或结构扭曲，导致人眼可疑、视觉质量下降，从而限制其实用性。本文希望在“非受限、可迁移、可攻击”的同时，让生成“更像真实图/自然风格图”。

本文的核心思想是：在将参考图像的风格迁移到原始干净图像上的过程中，有意地嵌入对抗扰动。具体而言，首先提取原始图像的内容表征与标签信息，以及参考图像的风格表征；随后通过持续学习这些特征信息来生成风格化的对抗图像。通过这种方式，生成的对抗图像能够有效诱导 DNN 做出错误预测，同时对原图仅引入风格层面的变化（例如颜色、亮度、对比度、笔触形状与细尺度纹理等），并尽可能保留其语义内容（例如目标对象、结构与空间布局）。

接下来，为了显著提升生成对抗图像的质量，作者根据不同的吸引性风格将参考图像划分为两类（照片与艺术作品），并进一步扩展 AdvST，设计两种相应的攻击策略。对于与原图语义内容相近但场景不同的自然照片参考图，我们设计了照片级真实感攻击（P-Attack）：该方法将语义分割模型提取的语义信息与风格表征相结合，以避免颜色溢出。在这种情况下，生成的对抗图像会呈现出照片级的风格化效果，仿佛拍摄于不同场景之中，例如不同光照、不同一天中的时间（阳光强度变化）、不同天气（云层厚度变化）、不同季节以及其他自然变化。对于具有知名艺术作品风格的参考图，我们设计了艺术风格攻击（A-Attack）：该方法通过引入拉普拉斯算子，在艺术风格融合与结构保持之间进行联合权衡，从而使生成的对抗图像既能保留艺术作品的细尺度特征，又能很好地保持原图内容。

<font size=4><b>本文核心方法：</b></font>

![](./assets/img/AdvST_fig1.png)
